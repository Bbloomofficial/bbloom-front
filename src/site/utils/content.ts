...
export function toMedia(value: unknown): MediaRef | undefined {
  ...
  if (typeof value === "string") {
    return value.trim()
      ? { id: null, url: assetUrl(value), width: null, height: null, alt: null }
      : undefined;
  }
  if (typeof value === "object") {
    ...
    if (typeof url === "string" && url.trim()) {
      return {
        ...
        // The API hands back a root-relative `/api/v1/media/{id}`, which only
        // resolves against the page by accident. Normalise here, the one
        // funnel every image in the renderer passes through.
        url: assetUrl(url),
