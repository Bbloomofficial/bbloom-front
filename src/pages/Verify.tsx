...
export default function Verify() {
  ...
  const [detail, setDetail] = useState<string | null>(
    token ? null : t.verify.missingToken,
  );
  ...
  return (
    <div className="flex min-h-screen flex-col bg-canvas">
      ...
      <main className="container-page flex flex-1 items-center justify-center py-10">
        <div className="w-full max-w-md rounded-3xl border border-ink-100 bg-surface p-6 text-center shadow-xl shadow-bloom-600/5 sm:p-8">
          {state === "checking" ? (
            <p className="text-sm text-ink-400">{t.verify.pageChecking}</p>
          ) : state === "done" ? (
            <>
