...
/**
 * Staff sessions are stored under their own key. The client dashboard keeps its
 * site-scoped session in `bbloom:site-session`, and the two must never share:
 * signing in here would otherwise sign a client out of their own website.
 */
const STORAGE_KEY = "bbloom:staff-session";

type StoredSession = { token: string; expiresAt: string; user: StaffProfile };
...
type AuthValue = {
  ...
  signOut: () => void;
  /** Signs out on an expired or rejected token, otherwise does nothing. */
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Revalidate a restored token once, so a revoked session lands on the login
  // screen rather than on an admin area full of failed requests.
  useEffect(() => {
...
export function useSession() {
  ...
  if (!auth.token || !auth.user) {
    throw new Error("useSession requires an authenticated admin route");
  }
